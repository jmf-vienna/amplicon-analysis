...
jmf::quiet()
options(warn = as.integer(Sys.getenv("R_OPTIONS_WARN", "2")))
tar_option_set(
...
list(
  ...
  tar_target(
    ...
    biosample_metrics |>
      filter(
        is.na(rank) | rank == feature_id_var,
        if_all(any_of("subset"), is.na),
  ...
  tar_target(deseq_results, finalize_tests_table(deseq_combined_results)),
  tar_target(
    feature_tests_file,
